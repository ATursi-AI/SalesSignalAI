"""
Apify-based Facebook Groups monitor for SalesSignal AI.

Replaces the fragile Playwright-based scraper with Apify's cloud-hosted
Facebook Groups Scraper actor. No local browser needed.

Uses MonitoredFacebookGroup model for group configuration.
Dynamically works with any US geography — no hardcoded regions.
"""
import logging
from datetime import datetime, timedelta

from django.utils import timezone

from core.models.monitoring import MonitoredFacebookGroup, MonitorRun
from core.utils.apify_client import ApifyIntegration, ApifyError
from .lead_processor import process_lead

logger = logging.getLogger(__name__)

# Apify actor for Facebook Groups scraping
ACTOR_ID = 'apify/facebook-groups-scraper'

# Cooldown between runs
COOLDOWN_MINUTES = 60

# Service request signals (same as Playwright version)
SERVICE_SIGNALS = [
    'looking for', 'need a', 'anyone know', 'recommend', 'recommendation',
    'who do you use', 'referral', 'reliable', 'affordable',
    'plumber', 'electrician', 'hvac', 'contractor', 'handyman',
    'landscap', 'cleaning', 'cleaner', 'roofer', 'painter',
    'flooring', 'drywall', 'pest control', 'exterminator',
    'mold', 'water damage', 'repair', 'renovation',
    'kitchen remodel', 'bathroom remodel', 'basement',
    'gutter', 'siding', 'fence', 'deck', 'patio',
    'tree service', 'tree removal', 'snow removal',
    'moving company', 'movers', 'junk removal',
    'locksmith', 'garage door', 'window replacement',
    'looking to hire', 'need help finding',
    'any suggestions', 'can anyone recommend',
    'does anyone have a good',
]


def is_service_request(text, extra_keywords=None):
    """Check if post text matches service-request signals."""
    text_lower = text.lower()
    signals = SERVICE_SIGNALS
    if extra_keywords:
        signals = signals + [k.lower() for k in extra_keywords]
    return any(s in text_lower for s in signals)


def _parse_timestamp(raw):
    """Parse an Apify timestamp string into a timezone-aware datetime."""
    if not raw:
        return None
    try:
        if isinstance(raw, str):
            dt = datetime.fromisoformat(raw.replace('Z', '+00:00'))
        elif isinstance(raw, (int, float)):
            dt = datetime.fromtimestamp(raw, tz=timezone.utc)
        else:
            return None
        if timezone.is_naive(dt):
            dt = timezone.make_aware(dt)
        return dt
    except (ValueError, TypeError, OSError):
        return None


def monitor_facebook_apify(group_ids=None, max_posts_per_group=20,
                           max_age_hours=48, dry_run=False):
    """
    Monitor Facebook Groups via Apify cloud scraper.

    Args:
        group_ids: list of MonitoredFacebookGroup IDs (default: all active)
        max_posts_per_group: max recent posts to fetch per group
        max_age_hours: skip posts older than this
        dry_run: log matches without creating Lead records

    Returns:
        dict with counts: groups_checked, posts_scraped, created,
                         duplicates, assigned, errors
    """
    # Cooldown check
    last_run = (
        MonitorRun.objects
        .filter(monitor_name='facebook_apify', status__in=('success', 'partial'))
        .order_by('-finished_at')
        .first()
    )
    if last_run and last_run.finished_at:
        elapsed = timezone.now() - last_run.finished_at
        if elapsed < timedelta(minutes=COOLDOWN_MINUTES):
            remaining = int((timedelta(minutes=COOLDOWN_MINUTES) - elapsed).total_seconds() / 60)
            reason = f'facebook_apify cooldown: {remaining}m remaining'
            logger.info(reason)
            return {'groups_checked': 0, 'posts_scraped': 0, 'created': 0,
                    'duplicates': 0, 'assigned': 0, 'errors': 0,
                    'skipped_reason': reason}

    # Initialize Apify client
    try:
        apify = ApifyIntegration()
    except ApifyError as e:
        logger.error(f'Apify not available: {e}')
        return {'error': 'api_not_configured'}

    # Load groups to monitor
    groups = MonitoredFacebookGroup.objects.filter(is_active=True)
    if group_ids:
        groups = groups.filter(id__in=group_ids)

    if not groups.exists():
        logger.info('No active Facebook groups configured')
        return {'groups_checked': 0, 'posts_scraped': 0, 'created': 0,
                'duplicates': 0, 'assigned': 0, 'errors': 0}

    stats = {
        'groups_checked': 0,
        'posts_scraped': 0,
        'created': 0,
        'duplicates': 0,
        'assigned': 0,
        'errors': 0,
    }

    cutoff = timezone.now() - timedelta(hours=max_age_hours)

    for group in groups:
        stats['groups_checked'] += 1
        logger.info(f'[Apify FB] Scraping group: {group.name}')

        # Build actor input for this group
        run_input = {
            'startUrls': [{'url': group.url}],
            'maxPosts': max_posts_per_group,
            'maxComments': 0,
        }

        try:
            items = apify.run_actor(ACTOR_ID, run_input, timeout_secs=180)
        except ApifyError as e:
            logger.error(f'[Apify FB] Failed scraping {group.name}: {e}')
            stats['errors'] += 1
            continue

        stats['posts_scraped'] += len(items)
        group_leads = 0

        for item in items:
            try:
                text = item.get('text', '') or item.get('message', '') or ''
                if not text or len(text) < 20:
                    continue

                posted_at = _parse_timestamp(
                    item.get('time') or item.get('timestamp') or
                    item.get('date') or item.get('createdTime')
                )

                # Skip old posts
                if posted_at and posted_at < cutoff:
                    continue

                # Filter for service requests
                if not is_service_request(text, extra_keywords=group.keywords):
                    continue

                author = (
                    item.get('userName') or item.get('user', {}).get('name', '')
                    if isinstance(item.get('user'), dict)
                    else item.get('userName', '')
                ) or ''
                post_url = item.get('url') or item.get('postUrl') or group.url

                content = f"[FB Group: {group.name}]\n{text[:2000]}"

                if dry_run:
                    logger.info(f'[DRY RUN] Would create lead: {text[:80]}')
                    stats['created'] += 1
                    group_leads += 1
                    continue

                lead, created, num_assigned = process_lead(
                    platform='facebook',
                    source_url=post_url,
                    content=content,
                    author=author,
                    posted_at=posted_at,
                    raw_data={
                        'group_name': group.name,
                        'group_id': group.group_id,
                        'post_text': text[:500],
                        'source': 'apify',
                    },
                )

                if created:
                    stats['created'] += 1
                    stats['assigned'] += num_assigned
                    group_leads += 1
                else:
                    stats['duplicates'] += 1

            except Exception as e:
                logger.error(f'[Apify FB] Error processing post: {e}')
                stats['errors'] += 1

        # Update group stats
        group.last_scraped = timezone.now()
        group.posts_scraped += len(items)
        group.leads_created += group_leads
        group.save(update_fields=['last_scraped', 'posts_scraped', 'leads_created'])

    logger.info(f'Facebook Apify monitor complete: {stats}')
    return stats
