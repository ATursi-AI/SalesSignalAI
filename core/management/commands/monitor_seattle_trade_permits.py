"""
Seattle Trade Permits Monitor
API: https://data.seattle.gov/resource/c87v-5hwh.json  (Socrata SODA)
Dataset: Trade permits (boiler, pressure vessel, mechanical, refrigeration, conveyance/elevator,
fire sprinkler, furnace, sign, side sewer permits)

Rich fields:
  - permitnum, permitclass, permitclassmapped, permittype, permittypemapped
...
SODA_URL = 'https://data.seattle.gov/resource/c87v-5hwh.json'
...
class Command(BaseCommand):
    help = 'Monitor Seattle Trade Permits (Socrata)'
    ...
    def handle(self, *args, **options):
        ...
        run = MonitorRun.objects.create(
            monitor_name='seattle_trade_permits',
            details={'days': days, 'limit': limit},
        )

        since = (datetime.now() - timedelta(days=days)).strftime('%Y-%m-%d')

        # Query Seattle trade permits
        params = {
        ...
        self.stdout.write(f"\n{'='*60}")
        self.stdout.write(f"  SEATTLE TRADE PERMITS MONITOR")
        ...
        try:
            ...
            for rec in records:
                ...
                if estprojectcost > 100000:
                    ...
                    urgency = 'new'
                    urgency_note = 'Trade permit issued'
                ...
                # Build rich content
                content_parts = [f'SEATTLE TRADE PERMIT: {permitclassmapped or permitclass}']
                ...
                try:
                    lead, created, num_assigned = process_lead(
                        ...
                        posted_at=posted_at,
                        raw_data={
                            'data_source': 'seattle_trade_permits',
                            'permitnum': permitnum,
                            'contractor': contractor,
                            'address': full_addr,
                            'permit_class': permitclassmapped or permitclass,
                            'permit_type': permittypemapped or permittype,
                    ...
                    logger.error(f"Seattle trade permit error for {display_name}: {e}")
                    stats['errors'] += 1

        except Exception as e:
            logger.error(f"Seattle trade permits error: {e}")
            stats['errors'] += 1
