"""
Monitor Tucson / Pima County AZ health inspections.

Usage:
    python manage.py monitor_pima_health --days 7 --dry-run
    python manage.py monitor_pima_health --days 14
"""
from django.core.management.base import BaseCommand
from core.models.monitoring import MonitorRun
from core.utils.monitors.pima_health import monitor_pima_health


class Command(BaseCommand):
    help = (
        'Monitor Pima County (Tucson) food facility inspections. '
        'Portal confirmed current March 2026. Mountain time zone.'
    )
    ...
    def handle(self, *args, **options):
        run = MonitorRun.objects.create(
            monitor_name='pima_health',
            details={'days': options['days']},
        )

        self.stdout.write(f"\n{'='*60}")
        self.stdout.write(f"  TUCSON / PIMA COUNTY HEALTH INSPECTION MONITOR")
        self.stdout.write(f"  Days: {options['days']}")
        if options['dry_run']:
            self.stdout.write(self.style.WARNING('  DRY RUN MODE'))
        self.stdout.write(f"{'='*60}\n")

        stats = monitor_pima_health(
            days=options['days'],
