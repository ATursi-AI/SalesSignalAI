"""
Management command to run the Yelp competitor review monitor.
Usage:
    python manage.py monitor_yelp_reviews
    python manage.py monitor_yelp_reviews --dry-run
    python manage.py monitor_yelp_reviews --max-age 168
"""
from django.core.management.base import BaseCommand

from core.utils.monitors.yelp_reviews import monitor_yelp_reviews


class Command(BaseCommand):
    help = 'Monitor Yelp reviews on tracked competitors for opportunity leads'
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.HTTP_INFO('Starting Yelp review monitor...'))

        if options['dry_run']:
            self.stdout.write(self.style.WARNING('  DRY RUN MODE — no leads will be created'))

        stats = monitor_yelp_reviews(
            max_age_hours=options['max_age'],
            dry_run=options['dry_run'],
        )

        self.stdout.write('')
        self.stdout.write(self.style.SUCCESS('Yelp Review Monitor Results:'))
